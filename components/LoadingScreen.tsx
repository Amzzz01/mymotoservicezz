...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ fadeOut = false }) => {
  ...
  // fadeOut means data is actually ready and we're just playing the exit
  // transition, so stop claiming we're still loading once that starts.
  const isLoading = !fadeOut;

  useEffect(() => {
    if (!isLoading) return;
    const interval = setInterval(() => {
      setDotFrame((frame) => (frame + 1) % DOT_FRAMES.length);
    }, 450);
  ...
  return (
    ...
    >
      ...
      {/* Data-loading status, hidden once ready (fadeOut) so it doesn't linger during the exit transition */}
      {isLoading && (
        <div className="mt-3.5 flex items-center justify-center">
          <span aria-live="polite" className="text-xs text-slate-400 dark:text-slate-500">
            {t.splashLoadingData}
          </span>
          <span aria-live="off" className="text-xs text-slate-400 dark:text-slate-500">
            {DOT_FRAMES[dotFrame]}
          </span>
        </div>
      )}
